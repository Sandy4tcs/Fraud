...
public class SearchWs {
	...
	public String getCategory(@QueryParam("email") String email)
	{
		String json="";
		DBconnection database=new DBconnection();
		Connection connection=null;
		...
		try {
			connection = database.getConnection();
			query3="SELECT CATEGORY,COUNT(CATEGORY) AS COUNT1 FROM FRAUD_SCORE WHERE EMAIL=? GROUP BY CATEGORY";
	...
	String query4="",query5="";
	@GET
	@Path("categoryscore")
	@Produces("text/plain")
	@WebMethod(operationName = "category")
	public String getCategoryScore(@QueryParam("email") String email)
	{
		String json="";
		DBconnection database=new DBconnection();
		Connection connection=null;
		...
		try {
			connection = database.getConnection();
			query5="SELECT SUM(SCORE) AS SCORE FROM FRAUD_SCORE WHERE EMAIL=?";
			st = connection.prepareStatement(query5);
			st.setString(1, email);
			rs = st.executeQuery();
			while(rs.next())
			{
				ChartData cd=new ChartData();
				cd.setCategory("Consolidated Score");
				cd.setCount(rs.getInt("SCORE"));
				cdList.add(cd);
			}
			st.close();
			rs.close();
			query4="SELECT CATEGORY,SUM(SCORE) AS SCORE FROM FRAUD_SCORE WHERE EMAIL=? GROUP BY CATEGORY";
